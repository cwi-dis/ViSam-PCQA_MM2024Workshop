...
class TransformerEncoderLayer_CMA_cls(Module):
    ...
    def __init__(
        ...
        super(TransformerEncoderLayer_CMA_cls, self).__init__()
        ...
        self.pc2img_ca = TransformerEncoderLayer_GA_cls(
            d_model=d_model,
            nhead=nhead,
            dim_feedforward=dim_feedforward,
            dropout=dropout,
        )
        self.img2pc_ca = TransformerEncoderLayer_GA_cls(
            d_model=d_model,
            nhead=nhead,
            dim_feedforward=dim_feedforward,
            dropout=dropout,
    ...
    def forward(
        ...
        tex2D_global_attention = self.pc2img_ca(texture_img, texture_img)
        dep2D_global_attention = self.pc2img_ca(depth_img, depth_img)
        nor2D_global_attention = self.pc2img_ca(normal_img, normal_img)
        tex_img_global = torch.mean(texture_img, dim=1)
        depth_img_global = torch.mean(depth_img, dim=1)
        normal_img_global = torch.mean(normal_img, dim=1)
...
class TransformerEncoderLayer_CMA(Module):
    ...
    def forward(
        ...
        tex2D_dep2D_attention = self.pc2img_ca(texture_img, depth_img)
        dep2D_tex2D_attention = self.img2pc_ca(depth_img, texture_img)
        tex2D_nor2D_attention = self.pc2img_ca(texture_img, normal_img)
        nor2D_tex2D_attention = self.img2pc_ca(normal_img, texture_img)
        dep2D_nor2D_attention = self.pc2img_ca(depth_img, normal_img)
        nor2D_dep2D_attention = self.img2pc_ca(normal_img, depth_img)
...
class TransformerEncoderLayer_GA_cls(TransformerEncoderLayer_GA):
    def __init__(
        self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu", d_input=1024, bs=2
    ):
        super(TransformerEncoderLayer_GA_cls, self).__init__(d_model, nhead, dim_feedforward, dropout, activation)
    ...
    def forward(
        ...
        #------------changed by ruihong and then commented by xuemei?
        # cls_tokens = self.cls_token.expand(src.shape[0], -1, -1) #[b, 1, 1024]
        # src = torch.cat((cls_tokens, src), dim=1) #[b, 7,1024]
        src2 = self.self_attn(
            guide, src, src, attn_mask=src_mask, key_padding_mask=src_key_padding_mask
        )[0]
        """
        ...
        guide = guide + self.dropout2(_guide)
        guide = self.norm2(guide)
        return guide[:,0,:] #only return the cls token
