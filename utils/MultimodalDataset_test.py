...
class MMDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        transformed_vs = torch.zeros(
            [img_length_read, vs_channel, img_height_crop, img_width_crop]
        )
        ...
        for i in range(img_length_read):
            ...
            vs_name = os.path.join(vs_dir, str(i) + ".png")  # xm: add vs
            ...
            try:
                read_frame, read_depth, read_normal, read_vs = self.random_crop(
                    read_frame, read_depth, read_normal, read_vs
                )
            except:
                raise
            ...
            print("before", type(read_vs))
            read_vs = transforms.ToTensor()(read_vs)
            print("after", type(read_vs))
